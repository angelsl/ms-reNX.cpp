...
namespace reNX {
    ...
    template <>
    int64_t NXNode::value<int64_t>() const {
        return _data->Type1;
    }

    template <>
    double NXNode::value<double>() const {
        return _data->Type2;
    }

    template <>
    std::string NXNode::value<std::string>() const {
        return _file->get_string(_data->Type3);
    }

    template <>
    nxstring NXNode::value<nxstring>() const {
        return _file->get_nxstring(_data->Type3);
    }

    template <>
    point NXNode::value<point>() const {
        return point(_data->Type4.X, _data->Type4.Y);
    }

    template <>
    char* NXNode::value<char *>() const {
        // TODO
        return nullptr;
    }
